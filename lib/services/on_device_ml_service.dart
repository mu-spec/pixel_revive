...
class FaceRegion {
  ...
  factory FaceRegion.fromFace(Face face) {
    ...
    return FaceRegion(
      ...
      rightEye: getLandmarkOrFallback(
...
class OnDeviceMlService {
  ...
  static void initialize() {
    ...
    debugPrint(
      "🧠 On-Device ML Kit Face Detector initialized with Landmarks (fully offline!)",
    );
  }

  static String _generateCacheKey(Uint8List imageBytes) {
    final keyBytes =
        imageBytes.length > 1024 ? imageBytes.sublist(0, 1024) : imageBytes;
  ...
  static Future<List<FaceRegion>> detectFaceRegions(
    ...
    if (!forceRefresh && _regionCache.containsKey(cacheKey)) {
      ...
      if (timestamp != null &&
        ...
        debugPrint(
          "🧠 Face Detection: Using cached regions (${_regionCache[cacheKey]!.length} faces)",
        );
    ...
    try {
      ...
      final tempFile = File(
        '${cacheDir.path}/ml_face_${DateTime.now().millisecondsSinceEpoch}.jpg',
      );
      ...
      final List<FaceRegion> regions =
          faces.map((face) => FaceRegion.fromFace(face)).toList();

      _regionCache[cacheKey] = regions;
      _cacheTimestamps[cacheKey] = DateTime.now();

      debugPrint(
        "🧠 Face Detection: Detected ${faces.length} face region(s) with landmarks",
      );
      return regions;
    } catch (e) {
  ...
  static Future<List<Rect>> detectFaces(
    ...
    final regions = await detectFaceRegions(
      imageBytes,
